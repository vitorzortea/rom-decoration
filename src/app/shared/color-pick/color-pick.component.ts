...
@Component({
  ...
  styleUrls: ['./color-pick.component.styl']
})
export class ColorPickComponent implements OnInit {
  ...
  viewGradiente(index, color) {
    ...
    const allGradiente = document.querySelectorAll(`.color-${this.local.slug} ul`);
    const selectGradiente = document.querySelector(`.color-${this.local.slug} ul:nth-child(${index + 1}n)`);
    allGradiente.forEach(e => { e.classList.remove('select'); });
    selectGradiente.classList.add('select');
  }

  selectColor(color) {
    this.local.cor = color;
  }
